    ...
    def ppom_crawl(self, wait_time, searching_page):
        ...
        def doit(before_data=None):
            ...
            if before_data is not None:
                ...
                temp_href = list()
                while True:
                    for n, page in enumerate(titles_href):
                        if before_data in page:
                            temp_href = titles_href[n:]
                            break
                    #페이지 넘어감
                    start = '&page='
                    end = '&'
                    start_number_of_page = page.rfind(start)+len(start)
                    end_number_of_page = page.rfind(end)

                    page_num = page[start_number_of_page:end_number_of_page]

                    url = page[:start_number_of_page] + page_num + page[end_number_of_page:]
                    driver.get(url)
                
                titles_href = temp_href
